...
update_and_render :: proc() {
	...
	if key_down(auto_cast 'W') {
		player.frame.input_axis.y += 1.0
	}
	if key_down(auto_cast 'A') {
		player.frame.input_axis.x -= 1.0
	}
	if key_down(auto_cast 'S') {
		player.frame.input_axis.y -= 1.0
	}
	if key_down(auto_cast 'D') {
		player.frame.input_axis.x += 1.0
	}
	...
	draw_rect_aabb(v2{ game_res_w * -0.5, game_res_h * -0.5}, v2{game_res_w, game_res_h}, img_id=.background_0)
